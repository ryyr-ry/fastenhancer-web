...
// dist/api/errors.js
var FastEnhancerError, WasmLoadError, ModelInitError, AudioContextError, WorkletError, ValidationError, DestroyedError;
var init_errors = __esm(() => {
  FastEnhancerError = class FastEnhancerError extends Error {
    code;
  ...
  WasmLoadError = class WasmLoadError extends FastEnhancerError {
  ...
  ModelInitError = class ModelInitError extends FastEnhancerError {
  ...
  AudioContextError = class AudioContextError extends FastEnhancerError {
  ...
  WorkletError = class WorkletError extends FastEnhancerError {
    code = "WORKLET_ERROR";
    constructor(message, options) {
      super(message, options);
  ...
  ValidationError = class ValidationError extends FastEnhancerError {
  ...
  DestroyedError = class DestroyedError extends FastEnhancerError {
...
// dist/api/wasm-loader.js
function selectWasmVariant(simdSupported) {
  return simdSupported ? "simd" : "scalar";
}

// dist/api/simd-detect.js
function getSimdTestBytes() {
...
var SIMD_TEST_MODULE;
var init_simd_detect = __esm(() => {
  SIMD_TEST_MODULE = [
    0,
    97,
    115,
...
// dist/api/stream-denoiser.js
async function createStreamDenoiser(options) {
  const { inputStream, wasmBytes, weightBytes, exportMap, modelSize, workletUrl, onWarning } = options;
  let audioContext;
  try {
  ...
  const denoiser = {
    ...
    getWorkletState() {
      ...
      return new Promise((resolve, reject) => {
        ...
        workletNode.port.addEventListener("message", handler);
        workletNode.port.start();
        workletNode.port.postMessage({ type: "get_state" });
      });
    }
  };
  return denoiser;
}
var TARGET_SAMPLE_RATE = 48000, WORKLET_INIT_TIMEOUT_MS = 1e4, WORKLET_STATE_TIMEOUT_MS = 3000;
var init_stream_denoiser = __esm(() => {
  init_errors();
});
...
var MODEL_CATALOG, RECOMMENDATIONS;
  ...
  const denoiser = {
    ...
    async switchModel(options) {
      ...
      const doSwitch = async () => {
        switching = true;
        try {
          const { model, weightData, modelSizeId = 0 } = options;
          ...
          try {
            ...
            newStatePtr = newWasm._fe_init(modelSizeId, weightPtr, weightLen);
          } finally {
...
async function createDenoiser(options) {
  const { model, weightData, modelSizeId = 0 } = options;
  ...
  try {
    ...
    statePtr = wasm._fe_init(modelSizeId, weightPtr, weightLen);
  } finally {
...
async function isSupported() {
  ...
  const audioWorkletSupported = typeof globalThis !== "undefined" && typeof globalThis.AudioWorkletNode === "function";
  return {
    wasm: wasmSupported,
    simd: simdSupported,
...
function loadModel(modelSize, options) {
  ...
  const cacheKey = `${modelSize}:${simd ?? "auto"}:${baseUrl ?? "default"}`;
...
async function loadModelImpl(modelSize, simdOption, baseUrlOption) {
  const simdSupported = simdOption !== undefined ? simdOption : await detectSimd2();
  const variant = selectWasmVariant(simdSupported);
  const wasmBase = baseUrlOption ? normalizeUrl(baseUrlOption) : new URL("../wasm/", import.meta.url).href;
  const weightBase = baseUrlOption ? normalizeUrl(baseUrlOption) : new URL("../../weights/", import.meta.url).href;
  ...
  const wasmJsUrl = `${wasmBase}${prefix}.js`;
  let wasmBytes;
  let weightData;
  let exportMap;
  try {
  ...
  return {
    ...
    async createDenoiser() {
      const { createDenoiser: create } = await Promise.resolve().then(() => (init_api(), exports_api));
      return create({
        model: {
          size: modelSize,
          sampleRate: 48000,
...
var VALID_MODELS, MODEL_SIZE_IDS, WEIGHT_FILENAMES, modelCache;
...
// tests/browser/react-test-app.ts
var import_react2 = __toESM(require_react(), 1);
var import_client = __toESM(require_client(), 1);

// dist/react/useDenoiser.js
init_loader();
init_errors();
var import_react = __toESM(require_react(), 1);
function useDenoiser(modelSize, options) {
  const [state, setState] = import_react.useState("idle");
  const [error, setError] = import_react.useState(null);
  const [outputStream, setOutputStream] = import_react.useState(null);
  const [bypass, setBypassState] = import_react.useState(false);
  const streamDenoiserRef = import_react.useRef(null);
  const destroyedRef = import_react.useRef(false);
  const requestIdRef = import_react.useRef(0);
  const modelSizeRef = import_react.useRef(modelSize);
  const optionsRef = import_react.useRef(options);
  modelSizeRef.current = modelSize;
  optionsRef.current = options;
  const cleanupStreamDenoiser = import_react.useCallback(() => {
  ...
  const start = import_react.useCallback(async (inputStream) => {
    ...
    try {
      ...
      const model = await loadModel(modelSizeRef.current, {
  ...
  const setBypass = import_react.useCallback((value) => {
  ...
  const destroy = import_react.useCallback(() => {
    if (destroyedRef.current)
      return;
...
// tests/browser/react-test-app.ts
function TestApp() {
  const inputCtxRef = import_react2.useRef(null);
  const { state, error, outputStream, bypass, start, stop, setBypass, destroy } = useDenoiser("tiny", {
    baseUrl: "/flat/",
    workletUrl: "/dist/worklet/processor.js"
  });
  const handleStart = import_react2.useCallback(async () => {
    ...
    try {
      ...
      ctx.close().catch((e) => console.warn("AudioContext close failed:", e));
      inputCtxRef.current = null;
    }
  }, [start]);
  const handleStop = import_react2.useCallback(() => {
    stop();
    if (inputCtxRef.current) {
      inputCtxRef.current.close().catch((e) => console.warn("AudioContext close failed:", e));
      inputCtxRef.current = null;
    }
  }, [stop]);
  const handleDestroy = import_react2.useCallback(() => {
    destroy();
    if (inputCtxRef.current) {
      inputCtxRef.current.close().catch((e) => console.warn("AudioContext close failed:", e));
      inputCtxRef.current = null;
  ...
  }, import_react2.default.createElement("button", { id: "btn-start", onClick: handleStart }, "Start"), import_react2.default.createElement("button", { id: "btn-stop", onClick: handleStop }, "Stop"), import_react2.default.createElement("button", { id: "btn-bypass-on", onClick: () => setBypass(true) }, "Bypass On"), import_react2.default.createElement("button", { id: "btn-bypass-off", onClick: () => setBypass(false) }, "Bypass Off"), import_react2.default.createElement("button", { id: "btn-destroy", onClick: handleDestroy }, "Destroy"));
}
async function main() {
