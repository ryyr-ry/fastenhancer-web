...
class CausalConv2d(nn.Conv2d):
    ...
    def __init__(
        ...
        kernel_size: tp.Tuple[int, int], freq: int,
        stride: int = 1,
        groups: int = 1, bias: bool = True,
        device=None, dtype=None
    ):
        self.freq = freq
        pad_freq = (kernel_size[1] - stride) // 2
        self.padding_time = kernel_size[0] - 1
        super().__init__(
            in_channels, out_channels, kernel_size,
            stride=(1, stride), padding=(0, pad_freq), dilation=1,
            groups=groups, bias=bias,
            device=device, dtype=dtype
        )

    def initialize_cache(self, x: Tensor) -> Tensor:
        return x.new_zeros(1, self.in_channels, self.padding_time, self.freq)
    ...
    def forward(self, x: Tensor, cache_in: tp.Optional[Tensor]) -> tp.Tuple[Tensor, Tensor]:
        ...
        if cache_in is not None:
            ...
            x = F.pad(x, (0, 0, self.padding_time, 0))
        cache_out = x[:, :, -self.padding_time:, :]
        return super().forward(x), cache_out


class ScaledCausalConvTranspose2d(nn.ConvTranspose2d):
    """ Input/output: [B, C, T, F] """
    def __init__(
        self,
        in_channels: int,
        ...
        super().__init__(
            in_channels, out_channels, kernel_size,
            stride=(1, stride), padding=(self.padding_time, pad_freq), dilation=1,
            groups=groups, bias=bias,
            device=device, dtype=dtype
        )
        self.normalize = normalize
    ...
    def remove_weight_reparameterizations(self):
        scale = self.scale.exp() if self.exp_scale else self.scale
        if self.normalize:
            weight = F.normalize(self.weight, dim=(0, 1, 2, 3)).mul_(scale)
        else:
            weight = self.weight * scale
        self.weight.data.copy_(weight)
        self.weight_norm = False
...
class ONNXModel(nn.Module):
    def __init__(
        self,
        channels: int = 64,
        kernel_size_freq: tp.List[int] = [8, 3, 3],
        ...
        # Encoder PreNet
        assert kernel_size_freq[0] % stride == 0
        assert (kernel_size_freq[0] - stride) % 2 == 0
        self.enc_pre = nn.Sequential(
            StridedConv1d(  # in_channels = 2 = [real, imag]
                2, channels, kernel_size_freq[0], stride=stride,
                padding=(kernel_size_freq[0] - stride) // 2, bias=False
            ),
        ...
        for idx in range(1, len(kernel_size_freq)):
            module = SequentialCache(
                CausalConv2d(
                    channels, channels, (kernel_size_time, kernel_size_freq[idx]),
                    stride=1, freq=freq, bias=False,
                ),
        ...
        self.rf_pre = nn.Sequential(
            rf_pre,
            nn.Conv2d(channels, self.rf_ch, 1, bias=False),
            BatchNorm2d(self.rf_ch),
        )
        ...
        self.rf_post = nn.Sequential(
            rf_post,
            nn.Conv2d(self.rf_ch, channels, 1, bias=False),
            BatchNorm2d(channels),
        )

        # Decoder
        self.decoder = nn.ModuleList()
        for idx in range(len(kernel_size_freq)-1, 0, -1):
            module = SequentialCache(
                ...
                CausalConv2d(
                    channels, channels,
                    (kernel_size_time, kernel_size_freq[idx]),
                    freq=freq, bias=False
                ),
                BatchNorm2d(channels),
                Act(**activation_kwargs),
                cache_idx=3
        ...
        self.dec_post = nn.Sequential(
            ...
            ScaledConvTranspose1d(
                channels, 2, kernel_size_freq[0], stride=stride,
                padding=(kernel_size_freq[0] - stride) // 2,
                bias=True, exp_scale=(final_scale == "exp"),
    ...
    def remove_weight_reparameterizations(self):
        ...
        """
        def merge_conv1d_bn(conv: nn.Module, norm: nn.Module, error_message: str = "") -> nn.Module:
        ...
        def merge_conv2d_bn(conv: nn.Module, norm: nn.Module, error_message: str = "") -> nn.Module:
            assert conv.bias is None, error_message
            std = norm.running_var.add(norm.eps).sqrt()
        ...
        # Encoder PreNet
        conv = merge_conv1d_bn(self.enc_pre[0], self.enc_pre[1], "enc_pre")
        self.enc_pre = nn.Sequential(conv, self.enc_pre[2])

        # Encoder
        new_encoder = nn.ModuleList()
        for idx, module in enumerate(self.encoder):
            conv = merge_conv2d_bn(module[0], module[1], f"enc.{idx}")
            new_module = SequentialCache(
                conv,       # Conv-BN Merged
                module[2],  # Activation
                cache_idx=0
            )
        ...
        # RNNFormer PreNet
        conv = merge_conv2d_bn(self.rf_pre[1], self.rf_pre[2], "rf_pre")
        self.rf_pre = nn.Sequential(
        ...
        # RNNFormer PostNet
        conv = merge_conv2d_bn(self.rf_post[1], self.rf_post[2], "rf_post")
        self.rf_post = nn.Sequential(
        ...
        for idx, module in enumerate(self.decoder):
            conv1 = merge_conv2d_bn(module[0], module[1], f"dec.{idx}.0")
            conv2 = merge_conv2d_bn(module[3], module[4], f"dec.{idx}.1")
            new_module = SequentialCache(
        ...
        # Decoder PostNet
        conv = merge_conv1d_bn(self.dec_post[0], self.dec_post[1], "dec_post")
        self.dec_post = nn.Sequential(
            conv,               # Conv-BN Merged
    ...
    def initialize_cache(self, x: Tensor) -> tp.List[Tensor]:
        ...
        for module in self.encoder:
            cache_list.append(module.initialize_cache(x))
        for block in self.rf_block:
    ...
    def model_forward(self, spec_noisy: Tensor, *args) -> tp.Tuple[Tensor, tp.List[Tensor]]:
        ...
        if len(cache_in_list) == 0:
            num_cache_total = len(self.encoder) + len(self.rf_block) + len(self.decoder)
            cache_in_list = [None for _ in range(num_cache_total)]
        x = spec_noisy
        ...
        # Encoder PreNet
        x = self.enc_pre(x)    # [B*T, C1, F1]
        encoder_outs = [x]

        # Encoder
        _, C1, F1 = x.shape
        x = x.view(B, T, C1, F1).transpose(1, 2)    # [B, C1, T, F1]
        for module in self.encoder:
            x, cache_out = module(x, cache_in_list.pop(0))
            encoder_outs.append(x)      # [B, C1, T, F1]
            cache_out_list.append(cache_out)

        # RNNFormer
        x = self.rf_pre(x)              # [B, C2, T, F2]
        x = x.permute(2, 0, 3, 1)       # [T, B, F2, C2]
        x = x.contiguous()
        for block in self.rf_block:
            cache_in = cache_in_list.pop(0)
            x, cache_out = block(x, cache_in)   # [T, B, F2, C2]
            cache_out_list.append(cache_out)
        x = x.permute(1, 3, 0, 2)       # [B, C2, T, F2]
        x = self.rf_post(x)             # [B, C1, T, F1]

        # Decoder
        for module in self.decoder:
            x = torch.cat([x, encoder_outs.pop(-1)], dim=1)     # [B, 2*C1, T, F1]
            x, cache_out = module(x, cache_in_list.pop(0))      # [B, C1, T, F1]
            cache_out_list.append(cache_out)

        # Decoder PostNet
        x = x.transpose(1, 2).reshape(B*T, C1, F1)          # [B*T, C1, F1]
        x = torch.cat([x, encoder_outs.pop(-1)], dim=1)     # [B*T, 2*C1, F1]
        x = self.dec_post(x)                                # [B*T, 2, F1]

        # Mask
        x = x.reshape(B, T, 2, FREQ).permute(0, 3, 1, 2)    # [B, F1, T, 2]
        mask = self.mask(x).contiguous()    # [B, F1, T, 2]
...
def test():
    ...
    hparams = get_hparams("configs/ablation/time_kernel_b.yaml")
