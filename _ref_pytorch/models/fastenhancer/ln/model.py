...
class LayerNorm(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        diff = x.sub_(mean)
        variance = diff.square().mean(dim=(2, 3), keepdim=True)
...
class ONNXModel(nn.Module):
    def __init__(
        ...
        for idx in range(1, len(kernel_size)):
            module = nn.Sequential(
                nn.Conv1d(
                    ...
                    padding=(kernel_size[idx] - 1) // 2, bias=True
                ),
                nn.GroupNorm(1, channels),
                Act(**activation_kwargs),
            )
        ...
        self.rf_pre = nn.Sequential(
            rf_pre,
            nn.Conv1d(channels, self.rf_ch, 1, bias=True),
            nn.GroupNorm(1, self.rf_ch),
        )
        ...
        self.rf_post = nn.Sequential(
            rf_post,
            nn.Conv1d(self.rf_ch, channels, 1, bias=True),
            nn.GroupNorm(1, channels),
        )
        ...
        for idx in range(len(kernel_size)-1, 0, -1):
            module = nn.Sequential(
                nn.Conv1d(channels*2, channels, 1, bias=True),
                nn.GroupNorm(1, channels),
                Act(**activation_kwargs),
                nn.Conv1d(
                    channels, channels, kernel_size[idx],
