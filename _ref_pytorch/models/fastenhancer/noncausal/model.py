...
class RNNFormerBlock(nn.Module):
    def __init__(
        ...
        self.rnn = nn.GRU(channels, channels, bidirectional=True, batch_first=False)
        self.rnn_fc = nn.Linear(channels*2, channels, bias=False)
        ...
        if weight_norm:
            ...
            self.rnn = weight_norm_fn(self.rnn, name="weight_ih_l0_reverse")
            self.rnn = weight_norm_fn(self.rnn, name="weight_hh_l0_reverse")
    ...
    def remove_weight_reparameterizations(self):
        if self.weight_norm:
            ...
            remove_parametrizations(self.rnn, "weight_ih_l0_reverse")
            remove_parametrizations(self.rnn, "weight_hh_l0_reverse")
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x, _ = self.rnn(x)                  # [T, B*F, C*2]
        x = x.view(TIME, BATCH, FREQ, 2*CH) # [T, B, F, C]
...
class Model(nn.Module):
    ...
    def get_stft(
        ...
        window: tp.Optional[str], normalized: bool
    ) -> nn.Module:
    ...
    def model_forward(self, spec_noisy: Tensor) -> Tensor:
        # spec_noisy: [B, F, T, 2]
        x = spec_noisy
    ...
    def forward(self, noisy: Tensor) -> tp.Tuple[Tensor, Tensor]:
        ...
        mask = self.model_forward(spec_noisy)           # [B, F, T, 2]
